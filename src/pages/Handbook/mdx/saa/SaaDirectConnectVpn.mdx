...
<SectionSummary
  lead="온프레미스와 AWS를 연결하는 두 가지 방식입니다. 빠른 구축이면 VPN, 지연·대역폭 안정성이 중요하면 Direct Connect를 고려합니다."
  bullets={[
    "Site-to-Site VPN: 인터넷(VPN 터널) 경유, 가상 프라이빗 게이트웨이 + 고객 게이트웨이",
    "Direct Connect: 전용선(물리 연결), DX 로케이션·가상 인터페이스",
    "DX + VPN 백업: Primary DX 실패 시 VPN으로 폴오버해 고가용·비용 절감",
  ]}
/>
...
<KeyTable
  columns={["구분", "Site-to-Site VPN", "Direct Connect"]}
  rows={[
...
<KeyTable
  ...
  rows={[
    ...
    ["고가용 연결·비용 최소·장애 시 백업", "Direct Connect 1회선 + VPN 백업"],
    ["대용량·민감 데이터 전송(예: DataSync)", "DataSync over Direct Connect"],
  ]}
/>
