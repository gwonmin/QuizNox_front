...
<SectionSummary
  lead="스토리지 유형별 차이입니다. 단일 인스턴스 블록이면 EBS, 다중 인스턴스 공유 파일이면 EFS, Windows·Lustre 등 특수 요구면 FSx를 선택합니다."
  bullets={[
    "EBS: 블록 스토리지, 단일 인스턴스에만 연결",
    "EFS: 파일 스토리지(NFS), 다중 인스턴스 동시 마운트",
    "FSx: Windows·Lustre 등 용도별 파일 스토리지",
  ]}
/>
...
<KeyTable
  columns={["시나리오", "접근"]}
  rows={[
    ["여러 EC2가 같은 파일 공유(리눅스)", "EFS로 이전; ALB 뒤 인스턴스 간 일관된 파일."],
