...
<SectionSummary
  lead="로그 패턴을 메트릭으로 변환해 알람·대시보드에 활용하는 CloudWatch 기능입니다."
  bullets={[
    "로그 그룹에 Metric Filter를 정의하고, 지정한 패턴이 매칭될 때마다 카운트 등 메트릭 발생",
    "값 추출: 패턴에서 숫자·문자열을 캡처해 메트릭 값 또는 차원으로 사용 가능(예: 지연 시간 p90)",
    "해당 메트릭으로 CloudWatch 알람 설정 → SNS 알림, EC2 복구 액션 등 연동",
    "EC2 커스텀 차원: 에이전트 설정 파일에서 append_dimensions 지정",
  ]}
/>
...
<KeyTable
  ...
  rows={[
    ...
    ["로그 필드의 p90 지연 시간을 시간별로 모니터링", "Metric Filter로 해당 필드를 메트릭화한 뒤 통계(p90)로 알람·대시보드 구성"],
    ["웹 서버 로그에서 HTTP 404 응답 횟수 모니터링", "Metric Filter로 404 패턴 매칭 → 카운트 메트릭 생성 → 알람"],
...
<KeyTable
  ...
  rows={[
    ...
    ["로그 latency 필드의 p90을 시간별로 통계", "Metric Filter로 해당 필드 메트릭화. Contributor Insights·Subscription filter는 용도 다름."],
    ["웹 서버 404 횟수 모니터링(운영 효율 최우선)", "Metric Filter로 404 패턴 매칭 → 카운트 메트릭·알람. Lambda·스크립트는 오답."],
  ]}
/>
