...
export function KeyTable({ columns, rows, className }: KeyTableProps) {
  return (
    <div className={cn("my-4 overflow-x-auto", className)}>
      <table className="w-full text-xs border-collapse border border-border/60 rounded-md overflow-hidden">
        <thead className="bg-muted">
          <tr>
            {columns.map((col) => (
              <th
                key={col}
                className="border border-border/60 px-2 py-1 text-left font-semibold text-foreground"
              >
                {col}
              </th>
            ))}
