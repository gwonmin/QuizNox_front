# DNS (records, TTL, resolver)

DNS에서 **이름을 누가 물어보고, 누가 답하는지**, 그리고 **레코드·TTL**만 구분합니다.
...
| 용어 | 의미 |
|------|------|
| **Record** | 도메인에 대한 한 줄 정보. A(IP), AAAA(IPv6), CNAME(별칭), MX(메일) 등 |
| **TTL** | 이 응답을 **몇 초 동안 캐시해도 되는지**. Resolver가 TTL 동안 재사용 후 만료 시 다시 조회 |
